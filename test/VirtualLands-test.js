...
const VirtualLands = artifacts.require('VirtualLands.sol');

require('chai').use(require('chai-as-promised'))
  				.use(require('chai-bignumber')(BigNumber))
				.should();

contract('VirtualLands', accounts => {
	...
	describe('withdraw', function () {
		...
		describe('someone sells an artwork', function () {
			...
			it('seller will get only their balance, dedectus a fee of 1 finney', async function () {
				...
				var howMuchShouldReceive = new BigNumber(web3.toWei(123-1, "finney"));
				howMuchShouldReceive = howMuchShouldReceive.minus(gasCost);
	...
	describe('totalSupplyLimit', function () {
	    it('has an initial total supply limit equal to ten thousand.', async function () {
	    	const totalSupplyLimit = await token.totalSupplyLimit();
	    	totalSupplyLimit.should.be.bignumber.equal(10000);
		});
	...
	describe('initialPrice set', function () {
		...
		it('can set initial price to a new value', async function () {
			await token.setInitialPrice(web3.toWei(33, "finney"), {from: owner });				
			const initialPrice = await token.initialPrice();
			initialPrice.should.be.bignumber.equal(web3.toWei(33, "finney"));
		});
	});
